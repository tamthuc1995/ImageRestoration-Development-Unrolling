...
class Extractor(nn.Module):
    def __init__(self, kernel_size, stride, padding,  n_features_in, n_features_out, n_channels, device):
        ...
        self.down_sampling_kernel = Parameter(
            torch.ones((n_channels, 1, 2, 2), device=self.device, dtype=torch.float32) * (1/2),
            requires_grad=True,
        )
...
class MultiScaleMixtureGLR(nn.Module):
    ...
    def graph_frame_recalibrate(self, input_height, input_width):
        with torch.no_grad():
            dummy_features_patchs = torch.zeros(size=(1, 3, input_height, input_width), device=self.device, dtype=torch.float32)
    ...
    def apply_multi_scale_lightweight_transformer(self, patchs, list_graph_weights):
        ...
        for level in range(self.n_levels-1):
            ...
            # print(f"downsampled_patchs.shape={downsampled_patchs.shape}")
        ...
        for level in range(self.n_levels-2, -1, -1):
            ...
            # print(f"upsampled_patchs_concat.shape={upsampled_patchs.shape}")
        ...
        # print(f"upsampled_patchs_final.shape={upsampled_patchs.shape}")
    ...
    def forward(self, patchs):
        ...
        for level in range(self.n_levels-1):
            ...
            gW, gD = glr_module.extract_edge_weights(
                features_patchs.view((bz, glr_module.n_graphs, nfts//glr_module.n_graphs, h, w))
            )
